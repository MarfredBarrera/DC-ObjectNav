...
**Key concepts**:
- **Feature Field** (`EvidentialFeatureField`): a single hash-grid + MLP mapping 3D position → the CLIPSeg relevance vector (one sigmoid channel per prompt: query + distractors; `hash_feature_dim` is DERIVED as 1+K at PerceptionStack init — not a config knob), trained with an evidential (Normal-Inverse-Gamma) head. The NIG marginal yields **both** aleatoric and epistemic uncertainty in one forward pass — no ensemble (the previous multi-model ensemble, whose only role was empirical epistemic from prediction variance, was replaced by this).
- **`_HistoryBuffer`**: Flat pre-allocated CPU ring of `(history_buffer_capacity, 3)` points and `(history_buffer_capacity, hash_feature_dim)` features. New points always enter; once full, each new point overwrites a uniformly random existing slot. Bounded memory (~40 MB CPU at 500k × 17 channels), continuous fresh-data dominance, O(batch) sampling with no `torch.cat` over many small chunks.
...
**Control is split by mode.** SEARCH (pre-latch) runs the MPPI planner below. Once a detection **latches**, EXPLOIT hands locomotion to the pretrained **DD-PPO PointNav policy** (Wijmans et al. 2020; depth-only SE-ResNeXt101 + 2-layer LSTM, vendored dependency-free in `ddppo_policy.py` to bit-exactly match the `gibson-2plus-se-resneXt101-lstm1024` checkpoint): each replan, `DDPPONavigator.act(depth, pos, rotation, goal_world)` emits ONE native Habitat primitive, executed via `SimInterface.step_discrete` with the checkpoint's own training magnitudes (`DDPPO_FORWARD_M`=0.25 m / `DDPPO_TURN_DEG`=10° — deliberately NOT `cfg.discrete_*`). **Steering target vs. arrival test are different points, deliberately** ([`ExploitController`](DCON/src/planning/exploit.py)): DD-PPO steers at the goal SNAPPED onto reachable free floor ([`snap_to_reachable_free`](DCON/src/planning/utils.py)) — its pointgoal must be navigable to stay in-distribution — while arrival is judged on the RAW box projection, which is what episode success measures. The snap is re-computed every replan and adopted only when strictly closer to the raw goal (`cfg.exploit_snap_tighten`), so it tightens as floor is observed and can never drift away under the policy. Three arrival conditions, in order: (1) within `cfg.stop_distance_m` of the raw detection; (2) standing on the snapped cell (`cfg.exploit_nav_arrival_m`) AND within `cfg.exploit_nav_max_object_m` of the detection — without (2), an approach whose snap exceeds `stop_distance_m` can never terminate (the policy parks on its pointgoal and spins against an unsatisfiable test), and without that guard a sparse-occupancy snap metres from the object stops the run on a guaranteed miss; (3) stagnation — `cfg.exploit_stagnation_replans` consecutive replans without closing on the nav goal by `cfg.exploit_stagnation_eps_m`, gated to within `cfg.exploit_stagnation_gate_m` of it, since every non-arrival otherwise burns to the step cap and then drifts away from its own closest approach. DD-PPO's own trained STOP is disregarded, and that is now measured rather than assumed: as the SOLE terminator — with the depth fix AND a navigable snapped pointgoal, i.e. every condition in its favour — it self-stops on **34.4%** of probe approaches while still being in position on 89.1%, so SR collapses to 33.6%; added on top of the radius rules it changed nothing (0 cases either way). `cfg.exploit_use_policy_stop` enables it for re-measurement; leave it off. Action selection **samples** (never argmax — greedy locks into a turn_left/turn_right 2-cycle via the prev-action embedding).

**Line-of-sight waypoint steering** (`cfg.exploit_waypoint_m`, 0 = off): DD-PPO is a LOCAL, map-free controller — handed a goal in another room it drives the bearing, meets the wall beside the doorway and circles. This is the entire residual failure mode after the depth fix: every remaining probe failure was an object inside a small room behind a narrow doorway, approaching to 1.6–2.1 m and then leaving for good. It is NOT a goal-placement problem (re-aiming at the navmesh's own nearest reachable point fixed 2 of 8; dilating obstacles for clearance fixed 0 of 7) — the missing information is the ROUTE.

The rule is **line of sight first**: if the goal is in clear view (Bresenham over the BEV; unseen cells do NOT block — treating the unobserved as solid would invent detours around empty space; and the last ~0.3 m before the goal is exempt, since the goal is by construction the free cell nearest the target object, so every line to it grazes that object — without the exemption the straight-aim branch fires on 0% of replans), aim straight at it. Only when geometry actually blocks the view does it fall back to the obstacle-aware route ([`goal_distance_field`](DCON/src/planning/utils.py), descended by steepest descent), and then to the FARTHEST route point still in sight, capped at the lookahead and floored at 0.4 m — in the doorway case, the opening itself. Both guards are load-bearing: a sub-0.4 m pointgoal leaves the policy no room to act and chains of them stalled whole approaches. Arrival is always judged on the real goals, never the waypoint.

Measured on the 128-approach probe (both arms with the band fix below): SR **89.1% → 97.7%**, self-stop 93.0% → 99.2%, at a cost of ~7 extra steps per approach (25 → 32). Per scene 94.6% → 98.2% (Collierville) and 84.7% → **97.2%** (Corozal). 13 approaches fixed, 2 regressed.

**Depth input**: resized 512→256 with `nearest-exact` (bilinear fabricates phantom edge depths), then sensor-miss `0.0` pixels are handled per `cfg.ddppo_depth_miss` — **`zero`** (default), the checkpoint's training-native reading, since Gibson's training meshes hole the same way. That choice is load-bearing, not cosmetic: a miss is *unknown*, and what gets substituted is what the policy believes is there. The former default `far` (misses→max depth) turns a window into a phantom 10 m corridor the policy walks into; it cost 9 points of probe SR (79.7 → 89.1) and was the sole cause of BOTH full-loop navigation failures (`tv__Collierville__ep11`/`ep25`, which reproduce at 0/4 and 0/3 seeds under `far` and 4/4 and 3/3 under `zero`). `nearest` fills each miss from its nearest VALID pixel (a window ringed by 3 m wall reads 3 m, a floor hole ringed by 1 m floor reads 1 m); it ties `zero` on the probe and is the option to reach for if a scene shows the opposite failure — a real hole in a surface reading as a point-blank wall. The LSTM state resets once per latch (`ddppo_nav.reset()` when `detected` first flips).
...
**5. EXPLOIT-only A/B: the deterministic probe (`benchmarks/exploit_probe.py` + `benchmarks/probe_report.py`)**
```bash
python benchmarks/exploit_probe.py --gpu 0 --scenes Collierville Corozal \
    --goals-per-combo 2 --starts-per-goal 8 --max-steps 200 --out output/probe_myarm
python benchmarks/probe_report.py output/probe_baseline output/probe_myarm     # A/B, footprint-scored
```
Isolates the DD-PPO approach from everything stochastic upstream: it spawns the agent at navmesh starts sampled by GEODESIC distance (1.5–5 m) facing a dataset goal and runs ONLY the occupancy update + `ExploitController.step` + `step_discrete` — no MPPI, no detector, no field training — so the same arm reproduces exactly and an A/B delta is attributable. This exists because MPPI's stochasticity makes full-loop runs uncomparable at small n. The defaults above are 128 approaches (~20 min/arm on one GPU). Arrival-rule and depth knobs are CLI-overridable per arm (`--nav-arrival-m`, `--stop-distance-m`, `--nav-max-object-m`, `--stagnation-replans`, `--no-snap-tighten`, `--depth-miss`) so the canonical config stays untouched while sweeping.

The BEV band is derived per case from **that case's own spawn height** (`start[1] + 0.2 .. + 1.5`), not the episode's start. They differ (Collierville: episode −0.149 vs sampled 0.027), and a band starting 2 cm above the floor makes the floor plane itself read as obstacle — which silently corrupted every occupancy-dependent result in that scene. The no-waypoint arm is insensitive to it (the snap still works), so it went unnoticed until waypoint routing, which reads the same map, produced nonsense there. If a probe result disagrees with a standalone `run_case` reproduction, check the band first.

`probe_report.py` buckets each approach by WHY it ended (**arrived** / **parked** — reached the snapped cell but the stop rule couldn't see it / **orbiting** / **no_progress**) and scores against the goal's rectangular FOOTPRINT, the Gibson-val rule — never the centroid, which is pessimistic by up to half an object's diagonal. Note the probe *drives* at the centroid while production latches on a box-center surface projection, so probe snap distances run larger than production's.

`tools/analyze_exploit_nav.py <eval_dir>` is the full-loop counterpart: it splits `traj_log.jsonl` into EXPLOIT phases (one per distinct raw goal cell) and reports d0/dmin/dfinal against the goal the controller was actually handed, so a navigation failure is separated from a detector false positive.
...
- **Hash-grid training**: `hash_train_batch_size`, `hash_buffer_refresh_interval`, `hash_per_frame_cache_size`, `history_buffer_capacity`. (`hash_feature_dim` is **not** configurable — PerceptionStack derives it as 1 + len(distractors), i.e. 17 for the canonical bank, 16 when the query itself filters one out; it is deliberately absent from `_YAML_SCHEMA`.)
...
- **Detection**: `detected_persistence`, `stop_distance_m` (also EXPLOIT's arrival check), `exploit_redetect_interval` (replans between detector runs once latched; `<=0` = never re-detect after latching). EXPLOIT arrival/termination (all disable at `<=0`, see the EXPLOIT section): `exploit_nav_arrival_m`, `exploit_nav_max_object_m`, `exploit_snap_tighten`, `exploit_stagnation_replans` / `exploit_stagnation_eps_m` / `exploit_stagnation_gate_m`, plus `exploit_waypoint_m` (route-following lookahead) and `exploit_use_policy_stop` (off — DD-PPO's STOP self-stops on only 34% of approaches). Detection-classification gates (each disables at `<=0`): `detected_min_box_frac` / `detected_max_box_frac` (box area as a fraction of the image) and `detected_min_dist_m` / `detected_max_dist_m` (agent→object distance, m) — *too close* (near OR fills the frame) is ignored, *too far* (distant OR tiny) investigates but doesn't latch, *usable band* latches.
...
- **EXPLOIT turn-only stall** (DD-PPO spins without advancing): historically caused by (a) the turn-sign inversion in step_discrete (fixed — native semantics now), (b) 30° turns vs the 10° training convention (fixed — DDPPO_TURN_DEG), or (c) how depth sensor-miss `0.0` blocks are filled — see `cfg.ddppo_depth_miss` above; misses were bulk-remapped to far as part of the same three-fix batch as (a)/(b) and never validated alone, and that remap turned out to *cause* its own class of failure (walking into windows). Dump the depth frame first; if the miss block is a window/opening `far` is the suspect (this is what it was — default is now `zero`), if it is a hole in a surface try `nearest`.
- **EXPLOIT never terminates** (burns the whole step budget near the target): the arrival tests read a different point than the policy steers at. Check `snap_m` in the traj log — whenever the snap exceeds `stop_distance_m` the raw-goal test is unsatisfiable by construction, and termination depends on the nav-goal arrival (`exploit_nav_arrival_m`, guarded by `exploit_nav_max_object_m`) or the stagnation stop.
