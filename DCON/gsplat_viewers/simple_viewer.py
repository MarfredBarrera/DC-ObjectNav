...
def main(local_rank: int, world_rank, world_size: int, args):
    ...
    if args.ckpt is None:
        ...
        # --- FIX: Initialize dummy uncertainty for test data ---
        # This prevents the NameError if you run without a checkpoint
        N = len(means)
        uncertainty_sh = torch.zeros(N, 16, device=device) 
        # -----------------------------------------------------
        ...
        means, quats, scales, opacities, sh0, shN, uncertainty_sh = [], [], [], [], [], [], []
        for ckpt_path in args.ckpt:
            ...
            sh0.append(ckpt["sh0"])
            shN.append(ckpt["shN"])
            # --- FIX: Load uncertainty parameter ---
            if "uncertainty_sh" in ckpt:
                uncertainty_sh.append(ckpt["uncertainty_sh"])
            else:
                # Fallback for old models without uncertainty
                N_pts = ckpt["means"].shape[0]
                uncertainty_sh.append(torch.zeros(N_pts, 16, device=device))
            # ---------------------------------------
        ...
        uncertainty_sh = torch.cat(uncertainty_sh, dim=0)
    ...
    def viewer_render_fn(camera_state: CameraState, render_tab_state: RenderTabState):
        ...
        RENDER_MODE_MAP = {
            ...
            "depth(expected)": "ED",
            "alpha": "RGB",
            "uncertainty": "RGB",  # Hijack RGB mode for uncertainty
        }

        # --- NEW: Swap colors if rendering uncertainty ---
        if render_tab_state.render_mode == "uncertainty":
            # Rasterizer expects [N, K, 3]. Uncertainty is [N, K].
            # We treat it as grayscale by expanding the last dim to 3 (R=G=B).
            colors_to_render = uncertainty_sh.unsqueeze(-1).expand(-1, -1, 3)
        else:
            colors_to_render = colors
        # -------------------------------------------------
        ...
        render_colors, render_alphas, info = rasterization(
            ...
            scales,  # [N, 3]
            opacities,  # [N]
            colors_to_render,  # USE THE SWAPPED COLORS
            viewmat[None],  # [1, 4, 4]
            K[None],  # [1, 3, 3]
            width,
            height,
        ...
        # --- NEW: Grayscale Uncertainty ---
        elif render_tab_state.render_mode == "uncertainty":
            # 1. Extract 1st channel (R)
            u_map = render_colors[0, ..., 0:1]
            
            # 2. Apply Sigmoid (Squash to 0..1 range)
            u_map = torch.sigmoid(u_map)
            
            # 3. Expand to RGB (Grayscale)
            # Shape becomes (H, W, 3) where R=G=B
            renders = u_map.expand(-1, -1, 3).cpu().numpy()
